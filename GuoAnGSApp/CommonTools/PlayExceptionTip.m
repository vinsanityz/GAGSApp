...
@implementation PlayExceptionTip
...
+(void)hidePoplabel
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [UIView setAnimationDuration:ExceptionAppearWithDisappearTime];
        backView.alpha=0;
        [UIView commitAnimations];
        sleep(ExceptionAppearWithDisappearTime);
        popView.hidden=YES;
...
+(void)showPop
{
    dispatch_async(dispatch_get_main_queue(), ^{
        if([popStrArray count]>0)
        {
            ...
            [PlayExceptionTip performSelector:@selector(hidePoplabel) withObject:nil afterDelay:ExceptionShowTime];
        }
    ...
    if(str)
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            if(nil==popView)
            {
                ...
                [PlayExceptionTip performSelector:@selector(showPop) withObject:nil afterDelay:0];
            }
            else
            {
                if(nil==popStrArray)
