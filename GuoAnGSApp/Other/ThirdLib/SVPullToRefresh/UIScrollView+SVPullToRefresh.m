    ...
    if(!self.pullToRefreshView) {
        ...
        switch (position) {
            case SVPullToRefreshPositionTop:
                yOrigin = - SVPullToRefreshViewHeight - HiddenDistance;
                break;
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(svpullColorAction) name:ColorNoti object:nil];
        
        if ([[SingleColor sharedInstance].colorEdition isEqualToString:WHITE_EDITION]) {
            self.activityIndicatorViewStyle = UIActivityIndicatorViewStyleGray;
        }else{
            self.activityIndicatorViewStyle = UIActivityIndicatorViewStyleWhite;
            
        }
        self.textColor = HEXCOLOR(0x6d6d6d);
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(@"下拉尝试刷新...",),
                       NSLocalizedString(@"松手立即刷新...",),
...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        ...
        _titleLabel.text = NSLocalizedString(@"下拉尝试刷新...",);
...
- (void)setLastUpdatedDate:(NSDate *)newLastUpdatedDate {
    ...
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"上次更新时间: %@",), newLastUpdatedDate?[self.dateFormatter stringFromDate:newLastUpdatedDate]:NSLocalizedString(@"暂无",)];
}
...
- (void)setDateFormatter:(NSDateFormatter *)newDateFormatter {
    ...
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"上次更新时间: %@",), self.lastUpdatedDate?[newDateFormatter stringFromDate:self.lastUpdatedDate]:NSLocalizedString(@"暂无",)];
}
...
- (void)stopAnimating {
    ...
    NSLog(@"%f",self.originalBottomInset);
    NSLog(@"%f",self.originalTopInset);
}
