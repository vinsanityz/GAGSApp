...
- (void)layoutSubviews
{
    ...
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        ...
        size = CGSizeMake(self.imagePathsGroup.count * self.pageControlDotSize.width  + 9 * (self.imagePathsGroup.count - 1), self.pageControlDotSize.height);//
    }
    //居于中心点时的位置
    ...
    NSLog(@"x:%f\ny:%f\nwidth:%f\nheight:%f",x,y,size.width,size.height);
