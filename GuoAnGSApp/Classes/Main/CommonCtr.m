...
-(void)creatUI
{
    ...
    //取出single中的字体与颜色
    mainColor = [_single.colorDic objectForKey:FONT_MAIN_COLOR];
    secColor = [_single.colorDic objectForKey:FONT_SEC_COLOR];
    cellColor = [_single.colorDic objectForKey:BACK_CONTROL_COLOR];
    bgColor = [_single.colorDic objectForKey:BACK_SUPER_COlOR];
    normal =[[_single.fontDic objectForKey:NAORMAL_SIZE] floatValue];
    big = [[_single.fontDic objectForKey:BIG_SIZE] integerValue];
    small =  [[_single.fontDic objectForKey:SMALL_SIZE] integerValue];
    //设置颜色
    [self setCommonColor];
}
...
-(void)setCommonColor
{
    self.view.backgroundColor = [_single.colorDic objectForKey:BACK_SUPER_COlOR];
    _backSuperView.backgroundColor = [_single.colorDic objectForKey:BACK_SUPER_COlOR];
    self.navigationController.navigationBar.barTintColor = [_single.colorDic objectForKey:NAVCOLOR];

    self.nameLab.textColor = [_single.colorDic objectForKey:FONT_NAV_MAIN_COLOR];
    self.titleLabel.textColor = [_single.colorDic objectForKey:FONT_NAV_MAIN_COLOR];
}

//设置字体大小
-(void)serCommonFont{
    self.nameLab.font = [UIFont systemFontOfSize:[[_single.fontDic objectForKey:NAORMAL_SIZE] intValue]];
    self.titleLabel.font = [UIFont systemFontOfSize:[[_single.fontDic objectForKey:NAORMAL_SIZE] intValue]];
}
...
-(void)setTitleMessage:(NSString *)titleMessage {
    ...
    _titleLabel.textColor = [_single.colorDic objectForKey:FONT_NAV_MAIN_COLOR];
    _titleLabel.font = [UIFont systemFontOfSize:[[_single.fontDic objectForKey:NAORMAL_SIZE]intValue]];
...
-(void)setNavLeft_title
{
    ...
    [btn setTitleColor:[_single.colorDic objectForKey:FONT_SEC_COLOR] forState:UIControlStateNormal];
    btn.titleLabel.font = [UIFont systemFontOfSize:[[_single.fontDic objectForKey:SMALL_SIZE]intValue]];
    [btn sizeToFit];
    [btn addTarget:self action:@selector(goBack) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *nextItm = [[UIBarButtonItem alloc] initWithCustomView:btn];
    self.navigationItem.leftBarButtonItem = nextItm;
...
#pragma mark - <没有网，没有缓存时加载的警示view>
...
-(void)requestDevisePersonInfoForPersonUserName:(NSString *)userName nickName:(NSString *)nickName birthday:(NSString *)birthday gender:(NSString *)gender address:(NSString *)address
{
    ...
    [LYNetworkManager POST:GetCompletePersonInfo parameters:dict successBlock:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if ([self.commonReturnCode.returnCode isEqualToNumber:[NSNumber numberWithInt:0]]) {
            [ZCZTipsView showWithString: @"修改成功"];
            ...
            [self.navigationController popViewControllerAnimated:YES];
             ...
             [ZCZTipsView showWithString: self.commonReturnCode.returnMsg];
